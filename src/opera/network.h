...
typedef enum {IP, TCP, TCPACK, TCPNACK, NDP, NDPACK, NDPNACK, NDPPULL, NDPLITE, 
  NDPLITEACK, NDPLITEPULL, NDPLITERTS, ETH_PAUSE, RLB, 
  XPDATA, XPCREDIT, XPCTL, HBHDATA, SAMPLE} packet_type;
...
class Packet {
 ...
 public:
    ...
    inline uint32_t flow_id() const {return _flowid;}
    inline void set_flow_id(uint32_t id) {_flowid = id;}
    ...
    void set_earlyhop(int hop) {_earlyhop = hop;}
    int get_earlyhop() {return _earlyhop;}
    void set_chosenhop(int hop) {_chosenhop = hop;}
    int get_chosenhop() {return _chosenhop;}
    void set_tidalhop(int hop) {_tidalhop = hop;}
    int get_tidalhop() {return _tidalhop;}
    ...
    vector<pair<int,int>> get_path() {return _tor_path;}
    void add_hop(pair<int,int> tor) {_tor_path.push_back(tor);}
    void set_path(vector<pair<int,int>> path) { _tor_path = path;}
    ...
    virtual inline XPassSrc* get_xpsrc(){return NULL;}
    virtual inline XPassSink* get_xpsink(){return NULL;}
    // stuff used for RLB:
    ...
    void set_tmp_ts(simtime_picosec ts) {_tmpts = ts;}
    simtime_picosec get_tmp_ts() {return _tmpts;}

    //HbH
    inline void set_nextq(Queue *q) { _next_q = q; }
    inline Queue *nextq() { return _next_q; } //need this to update invariants if packet is dropped
 ...
 protected:
    ...
    DynExpTopology* _top;
    Queue* _next_q; //hbh only
    //packet path tracking
    vector<pair<int,int>> _tor_path;
    simtime_picosec _fabricts; //timestamp from nic sentout
    simtime_picosec _tmpts; //tmp timestamp
    ...
    int16_t _real_dst; // _dst is used for routing (it could be the "intermediate" host, if packet is sent on 2 hop path)
    int16_t _real_src; // _real_dst is examined by the RLB module to determine whether to re-enque for a second hop.

    int16_t _src_ToR; // we use this for routing since RTS packets can have a src node and "src ToR" that don't match

    int16_t _src; // sending host, used to index labels
    int16_t _dst; // destination host, used to index labels
    int16_t _slice_sent; // the topology slice during which the packet was sent, used to index labels
    int16_t _path_index; // along which of multiple paths the packet was sent, used to index labels
    int16_t _crtToR; // current ToR (used to compute next ToR before packet sent out on pipe)
    int8_t _crthop; // current hop, used to index lables
    bool _lasthop;
    int8_t _maxhops;
    int8_t _earlyhop;
    int8_t _chosenhop;
