...
int main(int argc, char **argv) {
    ...
    while (i<argc) {
        if (!strcmp(argv[i],"-o")){
	       ...
	       i++;
        } else if (!strcmp(argv[i],"-hbhd")){
	       hbh_D = atoi(argv[i+1]);
	       i++;
        } else if (!strcmp(argv[i],"-hbhp")){
	       hbh_P = atoi(argv[i+1]);
	       i++;
...
// this creates the Expander topology
    ...
    if (input.is_open()){
        ...
        while(!input.eof()){
            ...
            HbHSrc* flowSrc = new HbHSrc(eventlist, flow_src, flow_dst, top);
            //flowSrc->setCwnd(cwnd*Packet::data_packet_size()); // congestion window
            flowSrc->set_flowsize(flow_size); // bytes

            HbHSink* flowSnk = new HbHSink();
            //tcpRtxScanner.registerHbH(*flowSrc);
