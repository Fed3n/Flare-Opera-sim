...
int main(int argc, char **argv) {
    ...
    double utiltime = 1; // milliseconds
    int64_t rlbflow = 0; // flow size of "flagged" RLB flows
    int64_t cutoff = 0; // cutoff between NDP and RLB flow sizes. flows < cutoff == NDP.

    //HPCC parameters
    int max_stage = 0; //the lower, the more often window is updated
    float nn = 0.95; //expected utilization, the lower the easy to control queuesize (paper=0.95)
...
// this creates the Expander topology
#ifdef DYNEXP
    DynExpTopology* top = new DynExpTopology(queuesize, &logfile, &eventlist, INT, topfile);
#endif
    ...
    //sample every 10us
    //RTTSampler *rttsampler = new RTTSampler(eventlist, 1000*1000, 0, 7, top);
    ...
    if (input.is_open()){
        ...
        while(!input.eof()){
            ...
            TcpSrc* flowSrc = new HPCCSrc(NULL, NULL, eventlist, top, flow_src, flow_dst,
                max_stage, nn);
            flowSrc->set_cwnd(cwnd*Packet::data_packet_size()); // congestion window
    ...
    //UM->start(timeFromSec(0.00000545)); // print utilization every X milliseconds.
    UM->start(timeFromMs(utiltime)); // print utilization every X milliseconds.
