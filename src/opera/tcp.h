...
class TcpSrc : public PacketSink, public EventSource {
 ...
 public:
    TcpSrc(TcpLogger* logger, TrafficLogger* pktlogger, EventList &eventlist, DynExpTopology *top, 
            int flow_src, int flow_dst, bool tdtcp = false);
    ...
    void signalLoss(uint64_t seqno);
    ...
    void map_seqno_to_slice(uint32_t seqno);
    void set_flowsize(uint64_t flow_size_in_bytes) {
        assert(flow_size_in_bytes > 0);
        assert(_mss > 0);
        _flow_size = flow_size_in_bytes;
        //cout << "setting flowsize " << _flow_id << " " << _flow_size << endl;
        if (_flow_size < _mss)
    ...
    simtime_picosec _rtt, _rto, _min_rto, _mdev,_base_rtt;
    int _cap;
    ...
    void send_packets();
    unsigned get_max_cwnd();

    int _subflow_id;
map<uint64_t, int> _seqno_to_slice;
#ifdef TDTCP
 ...
 protected:
    ...
    map<uint64_t, simtime_picosec> _pending_rtt_samples;
    map<uint64_t, int> _pending_ef_hop;
...
class TcpSink : public PacketSink, public DataReceiver, public Logged {
 ...
 private:
    ...
    void send_ack(simtime_picosec ts,bool marked,int pktslice,
        uint64_t pktid, map<string, pktINT> ints, vector<pair<int,int>> path,
        bool bolt_inc);
