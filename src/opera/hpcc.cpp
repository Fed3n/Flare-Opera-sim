...
HPCCSrc::measureInflight(map<string, pktINT> ints, int slice) {
    ...
    if(max_u >= 0) {
        // if(get_flowid() == 0 || get_flowid() == 1) {
        //     cout << "HPCCWND " << get_flowid() << " " << _cwnd << " " << max_u << " " << _U << 
        //         " " << slice << " " << eventlist().now() << endl;
        // }
        //if we are in a new timeslice ignore previous weighted _U
        if(_is_new_slice) {
            _U = max_u; 
        } else {
...
HPCCSrc::receivePacket(Packet& pkt) 
{
    ...
    TcpSrc::_ssthresh[0] = _ssthresh;
    #else
    TcpSrc::_cwnd = _cwnd;
    TcpSrc::_ssthresh = _ssthresh;
    #endif
