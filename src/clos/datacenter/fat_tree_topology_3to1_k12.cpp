...
Queue* FatTreeTopology::alloc_src_queue(QueueLogger* queueLogger){
    if(qt==CREDIT) {
    return  new NICCreditQueue(speedFromMbps((uint64_t)HOST_NIC), memFromPkt(FEEDER_BUFFER), *eventlist, queueLogger);
  }
...
Queue* FatTreeTopology::alloc_queue(QueueLogger* queueLogger, uint64_t speed, mem_b queuesize){
	...
	return new CreditQueue(speedFromMbps(speed), queuesize, *eventlist, queueLogger);
...
void FatTreeTopology::init_network(){
  ...
  for (int k = 0; k < Nup; k++) { // sweep upper pod switch id
    for (int l = 0; l < 6; l++) { // sweep lower pod
      ...
      queues_nup_nlp[k][j]->setName("UP" + ntoa(k) + "->LP" + ntoa(j));
  ...
  for (int j = 0; j < Nup; j++) { // sweep upper pod switch ID
    ...
    for (int l = 0; l < 6; l++) {
      ...
      int k = switch_pos * 6 + l; // get the core switch ID
    	...
    	queues_nup_nc[j][k]->setName("UP" + ntoa(j) + "->CS" + ntoa(k));
    	logfile->writeName(*(queues_nup_nc[j][k]));
    	
    	pipes_nup_nc[j][k] = new Pipe(timeFromNs(RTT_net), *eventlist);
    	pipes_nup_nc[j][k]->setName("Pipe-UP" + ntoa(j) + "->CS" + ntoa(k));
     ...
     // 	else
      queues_nc_nup[k][j] = alloc_queue(queueLogger, HOST_NIC, _queuesize);
    	queues_nc_nup[k][j]->setName("CS" + ntoa(k) + "->UP" + ntoa(j));
      logfile->writeName(*(queues_nc_nup[k][j]));
      
      pipes_nc_nup[k][j] = new Pipe(timeFromNs(RTT_net), *eventlist);
      pipes_nc_nup[k][j]->setName("Pipe-CS" + ntoa(k) + "->UP" + ntoa(j));
  ...
  : EventSource(eventlist,"utilmonitor"), _top(top)
{
    ...
    uint64_t rate = 1E11 / 8; // bytes / second
    rate = rate * _H;
...
UtilMonitor::UtilMonitor(FatTreeTopology* top, EventList &eventlist, map<uint64_t, TcpSrc*> flow_map)
...
UtilMonitor::findIdealShare() {
    ...
    for(vector<Queue*> vq : _top->queues_nc_nup) {
	for(Queue* q : vq) {
        if(!q) continue;
    ...
    for(vector<Queue*> vq : _top->queues_nup_nlp) {
	for(Queue* q : vq) {
        if(!q) continue;
        set<uint64_t> flows = q->getFlows();
	for(uint64_t id : flows) {
    ...
    for(vector<Queue*> vq : _top->queues_nlp_ns) {
	for(Queue* q : vq) {
        if(!q) continue;
        set<uint64_t> flows = q->getFlows();
	for(uint64_t id : flows) {
    ...
    for(vector<Queue*> vq : _top->queues_nup_nc) {
	for(Queue* q : vq) {
        if(!q) continue;
        set<uint64_t> flows = q->getFlows();
	for(uint64_t id : flows) {
    ...
    for(vector<Queue*> vq : _top->queues_nlp_nup) {
	for(Queue* q : vq) {
        if(!q) continue;
        set<uint64_t> flows = q->getFlows();
	for(uint64_t id : flows) {
    ...
    for(vector<Queue*> vq : _top->queues_ns_nlp) {
	for(Queue* q : vq) {
        if(!q) continue;
        set<uint64_t> flows = q->getFlows();
	for(uint64_t id : flows) {
    ...
    std::sort(max_collisions.begin(), max_collisions.end(), std::greater<>()); 
...
void
UtilMonitor::reportQueuesize() {
    int i = 0;
    for(vector<Queue*> vq : _top->queues_nc_nup) {
	for(Queue* q : vq) {
        if(!q) continue;
	cout << "Queue Core-Pod " << i++ << " " << q->reportMaxqueuesize() << " " <<
	    q->num_drops() << " " << q->tot_drops(); 
    q->reportLoss();
    cout << endl;
	q->reset_drops();
	}
	...
	for(Queue* q : vq) {
        if(!q) continue;
	cout << "Queue Pod-Core " << i++ << " " << q->reportMaxqueuesize() << " " <<
	    q->num_drops() << " " << q->tot_drops(); 
    q->reportLoss();
    cout << endl;
	q->reset_drops();
	...
	for(Queue* q : vq) {
        if(!q) continue;
	cout << "Queue Pod-ToR " << i++ << " " << q->reportMaxqueuesize() << " " <<
	    q->num_drops() << " " << q->tot_drops(); 
    q->reportLoss();
    cout << endl;
	q->reset_drops();
	...
	for(Queue* q : vq) {
        if(!q) continue;
	cout << "Queue ToR-Pod " << i++ << " " << q->reportMaxqueuesize() << " " <<
	    q->num_drops() << " " << q->tot_drops(); 
    q->reportLoss();
    cout << endl;
	q->reset_drops();
	...
	for(Queue* q : vq) {
        if(!q) continue;
	cout << "Queue ToR-Host " << i++ << " " << q->reportMaxqueuesize() << " " <<
	    q->num_drops() << " " << q->tot_drops(); 
    q->reportLoss();
    cout << endl;
	q->reset_drops();
	...
	for(Queue* q : vq) {
        if(!q) continue;
	cout << "Queue Host-ToR " << i++ << " " << q->reportMaxqueuesize() << " " <<
	    q->num_drops() << " " << q->tot_drops(); 
    q->reportLoss();
    cout << endl;
	q->reset_drops();
