...
loadfrac0=.30; % fraction of theoretically possible load

% ----- define simulation length (time):

totaltime=0.301; % seconds, probably want a little extra here...

filename=sprintf('websearch_%.0fpercLoad_300msec_%dhosts',100*loadfrac0,H);
...
% workload:
%data=csvread('../../traffic/w4_fb_hadoop_cdf.csv');
data=csvread('../../traffic/websearch.csv');
%data=csvread('../../traffic/boltrpc.csv');
%data=csvread('../../traffic/fbcoco.csv');
%data=csvread('../../traffic/flows15kb.csv');
flowsize=data(:,1).';
flowcdf=data(:,2).';
